...
3. **Async callback** – Configure `AIRALO_ASYNC_WEBHOOK_URL=https://<domain>/api/airalo/webhooks` or ensure `NEXT_PUBLIC_APP_URL`/Vercel exposes a public app URL so Simplify can derive it. Set a URL-safe `AIRALO_WEBHOOK_SECRET`; Simplify appends it as `airalo_webhook_secret` because Airalo async callbacks may arrive without `x-airalo-signature`.
