...
| `webhook_url`         | string         | ⚠️       | Optional override for the async callback target. Simplify always sends a per-request callback URL: `AIRALO_ASYNC_WEBHOOK_URL` when configured, otherwise `/api/airalo/webhooks` derived from a public app/Vercel URL. |
...
3. **Async callback** – Configure `AIRALO_ASYNC_WEBHOOK_URL=https://<domain>/api/airalo/webhooks` or ensure `NEXT_PUBLIC_APP_URL`/Vercel exposes a public app URL so Simplify can derive it. Set a URL-safe `AIRALO_WEBHOOK_SECRET`; async order submission fails before calling Airalo if this token is missing. Simplify appends it as `airalo_webhook_secret` because Airalo's async order docs do not define a webhook signing header.
4. **Async correlation** – Persist `request_id` from the `202 Accepted` response and map it to `payload.data.reference`, `payload.data.request_id`, or `payload.data.requestId` from the webhook. Checkout-created orders also include `simplify_order_id:<local-order-id>` in the Airalo `description` so callbacks can still match if Airalo omits `request_id`.
