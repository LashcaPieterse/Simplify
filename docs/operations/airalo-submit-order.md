...
| `webhook_url`         | string         | ⚠️       | Optional override for the async callback target. Simplify sends `AIRALO_ASYNC_WEBHOOK_URL` on async requests, or derives `/api/airalo/webhooks` from a public app/Vercel URL, unless `AIRALO_ASYNC_WEBHOOK_GLOBAL_OPT_IN=true` confirms Airalo has a dashboard-level webhook configured. |
